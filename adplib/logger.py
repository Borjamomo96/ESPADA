...
class Logger:
    ...
    @classmethod
    def setup_logger(cls, output_dir=None, log_path="adpalmap.log", clear_logs=False, queue=None):
                ...
                try:
                    ...
                    if abs_output_dir in abs_log_path.parents:
                        ...
                        logger.warning(
                            ...
                            f"'{output_dir}'. Using default path."
                        )
                        final_log_path = output_dir / "log_dir/adpalmap.log"
                
                except Exception as e:
                    logger.warning(f"Error processing absolute path: {e}. Using default path.")
                    final_log_path = output_dir / "log_dir/adpalmap.log"
    ...
    @classmethod
    def get_logger(cls, output_dir=None, log_path="adpalmap.log", clear_logs=False, queue=None):
        
        if cls._logger_instance is None:
            log_path = cls.setup_logger(output_dir, log_path=log_path, clear_logs=clear_logs, queue=queue)
        return cls._logger_instance
